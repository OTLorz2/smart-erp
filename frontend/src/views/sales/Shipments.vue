<template>
  <div class="shipments-page">
    <el-card>
      ...
      <el-table :data="orders" v-loading="loading">
        ...
        <el-table-column label="操作" width="150">
          <template #default="{ row }">
            <el-button v-if="row.status === 'confirmed'" type="primary" size="small" @click="handleShip(row)">发货</el-button>
    ...
    <!-- 发货对话框 -->
    <el-dialog v-model="shipDialogVisible" title="发货确认" width="600px">
      <el-form :model="shipForm" label-width="80px">
        ...
        <el-table :data="orderItems">
          ...
          <el-table-column prop="delivered_qty" label="已发货" />
          <el-table-column label="本次发货">
            <template #default="{ row }">
              <el-input-number v-model="row.ship_qty" :min="0" :max="Number(row.quantity) - Number(row.delivered_qty)" :precision="2" size="small" />
            </template>
          </el-table-column>
...
import { getSalesOrders, getSalesOrderItems, shipOrder, getWarehouses } from '@/api'
...
async function loadData() {
  ...
  try {
    ...
    orders.value = res.data.filter((o: any) => o.status === 'confirmed' || o.status === 'shipped')
...
async function handleShip(order: any) {
  ...
  orderItems.value = itemsRes.data.map((i: any) => ({
    ...i,
    ship_qty: Number(i.quantity) - Number(i.delivered_qty || 0),
    warehouse_id: 1
  }))
...
async function submitShip() {
  ...
  try {
    const items = orderItems.value
      .filter(i => i.ship_qty > 0)
      .map(i => ({
        material_id: i.material_id,
        quantity: i.ship_qty,
        warehouse_id: i.warehouse_id
      }))
    await shipOrder(currentOrder.value.id, { items })
